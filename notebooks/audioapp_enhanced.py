...
class AudioAnalyzerApp(ctk.CTk):
    ...
    def export_text(self, text_type: str):
        ...
        if output_format == "LaTeX PDF":
            ...
            self.export_as_latex_pdf(text_type)  # Modified to support both types
        else:
            # Regular text export
            ...
            if file_path:
                ...
                try:
                    ...
                    with open(file_path, 'w', encoding='utf-8', errors='replace') as f:
                        ...
                        content = self.clean_text_for_export(content)
                        f.write(content)
    ...
    def export_as_latex_pdf(self, text_type="result"):
        """Export result or transcription as compiled LaTeX PDF"""
        if text_type == "transcription":
            if not self.transcribed_text:
                messagebox.showwarning("No Content", "No transcription to export")
                return
    ...
    def clean_text_for_export(self, text):
        """Clean problematic characters for export"""
        replacements = [
            ('\u2019', "'"),
            ('\u201c', '"'),
            ('\u201d', '"'),
    ...
    def create_network_plot(self):
        ...
        def generate_plot():
            try:
                ...
                node_sizes = [300 + G.nodes[node]['weight'] * 30 for node in G.nodes()]
                ...
                # Add title with clarification
                ax.set_title("Semantic Network: Spatial Position = Word2Vec Similarity | Edges = Co-occurrence in Transcript", 
                ...
                # Add axes labels for dimensions
                ax.text(0.02, 0.02, "← Semantic Similarity (Word2Vec) →", 
                       transform=ax.transAxes, fontsize=9, alpha=0.6)
                ax.text(0.02, 0.98, "← Semantic Similarity (Word2Vec) →", 
                       transform=ax.transAxes, fontsize=9, alpha=0.6, rotation=90, va='top')
                ...
                self.after(0, lambda: self.show_network_plot_window(plot_path))
    ...
    def simple_clustering(self, similarity_matrix, n_clusters):
        ...
        while len(set(clusters.values())) > n_clusters:
            max_sim = 0
            merge_pair = None
            
            for i in range(n_items):
                for j in range(i + 1, n_items):
                    if clusters[i] != clusters[j] and similarity_matrix[i][j] > max_sim:
                        max_sim = similarity_matrix[i][j]
                        merge_pair = (clusters[i], clusters[j])
            
            if merge_pair:
                old_cluster, new_cluster = merge_pair
                for i in range(n_items):
        ...
        return {i: cluster_map[clusters[i]] for i in range(n_items)}
    
    # Function to show network plot in new window
    def show_network_plot_window(self, plot_path):
        """Display network plot in a new window"""
        plot_window = ctk.CTkToplevel(self)
        plot_window.title("Semantic Network Analysis")
        ...
        label = ctk.CTkLabel(plot_frame, image=photo, text="")
